...
int main(int argc, char** argv) {
	...
	vector<vector<int> > sigma(L,vector<int>(L,1)); // cold start because all sigmas are equal to one
	double acc=0.0;
	...
	for (int t=0;t<(ntherm+nstep);t++) {
		...
		if ((t>=ntherm)&&(((t-ntherm) % nmeas)==0)) {
		    ...
		    m /= (double(L)*double(L));
			cout <<pow(m*m,0.5)<<endl;  // I can get |m| by doing the square root.
		}
